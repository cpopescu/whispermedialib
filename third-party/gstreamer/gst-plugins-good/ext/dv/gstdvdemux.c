    ...
    gint64 * dest_value)
{
   ...
   * We check all these conditions on a case-per-case basis.
...
gst_dvdemux_chain (GstPad * pad, GstBuffer * buffer)
{
  ...
  if (GST_CLOCK_TIME_IS_VALID (timestamp)) {
    ...
    /* WHISPERCAST BEGIN:
     * We compute the current frame based on the timestamp we received...
    */
    frame_offset = dvdemux->frame_offset;
    format = GST_FORMAT_DEFAULT;
    if (!gst_dvdemux_src_convert (dvdemux, dvdemux->videosrcpad,
              GST_FORMAT_TIME, dvdemux->time_segment.last_stop,
              &format, &dvdemux->frame_offset))
      return GST_FLOW_ERROR;
    if (frame_offset != dvdemux->frame_offset)
      GST_INFO_OBJECT (dvdemux, "offset changed: %llu -> %llu",
          frame_offset, dvdemux->frame_offset);
    /* WHISPERCAST END */
  }
