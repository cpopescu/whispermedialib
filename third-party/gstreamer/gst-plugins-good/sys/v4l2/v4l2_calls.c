...
gst_v4l2_fill_lists (GstV4l2Object * v4l2object)
{
  ...
  for (n = V4L2_CID_BASE;; n++) {
    ...
    if (n == V4L2_CID_LASTP1) {
      ...
      /* WHISPERCAST BEGIN
      Allow all the controls to be used through the color balance interface.
      break;
      WHISPERCAST END */
    }
    ...
    if (ioctl (v4l2object->video_fd, VIDIOC_QUERYCTRL, &control) < 0) {
      if (errno == EINVAL) {
        if (n < V4L2_CID_PRIVATE_BASE)
          ...
          /* WHISPERCAST BEGIN
          We don't break here unless we checked the first 128 private CIDs...
          */
          if ((n -V4L2_CID_PRIVATE_BASE) > 127)
            break;
          else
            continue;
          /* WHISPERCAST END */
