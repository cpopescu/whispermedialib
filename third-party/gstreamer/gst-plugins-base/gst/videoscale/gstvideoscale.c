    ...
    GstCaps * caps, GstCaps * othercaps)
{
  ...
  if (from_par && to_par) {
    ...
    gint width, height, owidth, oheight, good;
    good = 0;

    if (!gst_structure_get_int (outs, "width", &owidth)) {
      owidth = -1;
    }
    if (!gst_structure_get_int (outs, "height", &oheight)) {
      oheight = -1;
    }
    GST_DEBUG_OBJECT (base, "WSP owidth=%d, oheight=%d", owidth, oheight);
    ...
    GST_DEBUG_OBJECT (base, "WSP width=%d, height=%d", width, height);

    if (width > 0 && height > 0) {
      if (owidth > 0 && oheight > 0) {
        good = 1;
        GST_DEBUG_OBJECT (base, "WSP keeping owidth=%d, oheight=%d", owidth, oheight);
      } else
      if (owidth > 0) {
        oheight = (owidth*height)/width;
        if (gst_structure_has_field (outs, "height")) {
          gst_structure_fixate_field_nearest_int (outs, "height", oheight);
        }
        GST_DEBUG_OBJECT (base, "WSP adjusted height to owidth=%d, oheight=%d", owidth, oheight);
        good = 1;
      } else
      if (oheight > 0) {
        owidth = (oheight*width)/height;
        if (gst_structure_has_field (outs, "width")) {
          gst_structure_fixate_field_nearest_int (outs, "width", owidth);
        }
        GST_DEBUG_OBJECT (base, "WSP adjusted width to owidth=%d, oheight=%d", owidth, oheight);
        good = 1;
      }
    }

    if (!good) {
      GST_DEBUG_OBJECT (base, "WSP default");
      if (gst_structure_get_int (ins, "width", &width)) {
        if (gst_structure_has_field (outs, "width")) {
