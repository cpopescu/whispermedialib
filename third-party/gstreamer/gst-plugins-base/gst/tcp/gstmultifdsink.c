 ...
 */
 ...
 /* WHISPERCAST BEGIN:
 * If protocol is set to GDP, the multifdsink will send out a
 * modified NEW_SEGMENT event for each client fd.
 * 
 * This is needed in order to allow synchronization
 * in linked pipelines.
 * 
 * In the process, we updated the GSP handling, too...
 */
 /* WHISPERCAST END */
...
#define DEFAULT_GDP_EVENTS              FALSE
...
gst_multi_fd_sink_init (GstMultiFdSink * this, GstMultiFdSinkClass * klass)
{
  ...
  /* WHISPERCAST BEGIN:
  * We are using the not-deprecated GDP packetizer.
  */
  this->packetizer = gst_dp_packetizer_new(GST_DP_VERSION_1_0);
  /* WHISPERCAST END */
}
    ...
    GstTCPClient * client, const GstCaps * caps)
{
  ...
  * We are using the not-deprecated GDP packetizer.
  */
  if (!sink->packetizer->packet_from_caps (caps, sink->header_flags, &length, &header,
          &payload)) {
    GST_DEBUG_OBJECT (sink, "Could not create GDP packet from caps");
    return FALSE;
  }
    ...
    GstTCPClient * client, GstBuffer * buffer)
{
  ...
  if (G_UNLIKELY (send_streamheader)) {
    ...
    /* WHISPERCAST BEGIN:
     * Here we create a new segment event and queue it for
     * the client, if it's the case...
     */
    if (sink->protocol == GST_TCP_PROTOCOL_GDP) {
      if (sink->element.have_newsegment) {
        if (GST_BUFFER_TIMESTAMP(buffer) != GST_CLOCK_TIME_NONE) {
          GstSegment *segment = &sink->element.segment;
          GstEvent *new_segment = 
          gst_event_new_new_segment_full (FALSE,
          segment->rate, segment->applied_rate, segment->format,
          GST_BUFFER_TIMESTAMP(buffer), segment->stop, GST_BUFFER_TIMESTAMP(buffer));
          
          guint8 *header;
          guint8 *payload;
          guint len;
          if (!sink->packetizer->packet_from_event (new_segment, sink->header_flags, &len,
                  &header, &payload)) {
            GST_DEBUG_OBJECT (sink,
                "[fd %5d] could not create header from event, removing client",
                client->fd.fd);
            gst_event_unref (new_segment);
            return FALSE;
          }
          gst_event_unref (new_segment);
          ...
          gst_multi_fd_sink_client_queue_data (sink, client, (gchar *) header,
              len);
          gst_multi_fd_sink_client_queue_data (sink, client, (gchar *) payload,
    ...
    if (!gst_structure_has_field (s, "streamheader")) {
      ...
      for (i = 0; i < buffers->len; ++i) {
        ...
        if (sink->protocol == GST_TCP_PROTOCOL_GDP) {
          ...
          * We are using the not-deprecated GDP packetizer.
          */
          if (!sink->packetizer->header_from_buffer (buffer, sink->header_flags, &len,
                  &header)) {
            GST_DEBUG_OBJECT (sink,
                "[fd %5d] could not create header, removing client",
                client->fd.fd);
  ...
  if (sink->protocol == GST_TCP_PROTOCOL_GDP) {
    ...
    * We are using the not-deprecated GDP packetizer.
    */
    if (!sink->packetizer->header_from_buffer (buffer, sink->header_flags, &len, &header)) {
      GST_DEBUG_OBJECT (sink,
          "[fd %5d] could not create header, removing client", client->fd.fd);
      return FALSE;
    }
