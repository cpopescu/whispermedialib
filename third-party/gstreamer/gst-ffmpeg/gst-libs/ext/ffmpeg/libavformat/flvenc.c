...
static int get_audio_flags(AVCodecContext *enc){
    ...
    case    44100: sample_flags = FLV_SAMPLERATE_44100HZ; break;
    case    22050: sample_flags = FLV_SAMPLERATE_22050HZ; break;
    case    11025: sample_flags = FLV_SAMPLERATE_11025HZ;  break;
    case     8000: //nellymoser only
    case     5512: //not mp3
      if(enc->codec_id != CODEC_ID_MP3){
        sample_flags |= FLV_SAMPLERATE_SPECIAL;
        break;
      }
    default:
    ...
    if (enc->codec_id == CODEC_ID_AAC) // specs force these parameters
        return flags | FLV_CODECID_AAC | FLV_SAMPLESSIZE_16BIT;
  ...
  for (i = 0; i < s->nb_streams; i++) {
    ...
    if (enc->codec_id == CODEC_ID_AAC || enc->codec_id == CODEC_ID_H264) {
      ...
      if (enc->codec_id == CODEC_ID_AAC) {
        ...
        if ( enc->extradata_size > 0 )  {
          put_buffer(pb, enc->extradata, enc->extradata_size);
        } else {
/*
          // Extra data misses - we need to figure it out ..
          ...
          uint16_t pattern = 0x2800;    // AAC HE..
          ...
          default:
            av_log(enc, AV_LOG_ERROR, "************ BAD BAD BAD: "
                   "unsupported sample rate - most probably this will fail..");
          }
          ...
          // END HACK END HACK END HACK END HACK END HACK END HACK END HACK
*/
        }
        //
      } else {
...
//    av_log(s, AV_LOG_DEBUG, "type:%d pts: %"PRId64" size:%d\n", enc->codec_type, timestamp, size);
    ...
    } else if (enc->codec_id == CODEC_ID_VP6F) {
