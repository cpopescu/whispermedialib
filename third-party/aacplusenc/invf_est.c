...
static __thread float quantStepsSbr[4]  = {1, 10, 14, 19};
static __thread float quantStepsOrig[4] = {0,  3,  7, 10};
static __thread float nrgBorders[4] =     {25.0f, 30.0f, 35.0f, 40.0f};

static __thread DETECTOR_PARAMETERS detectorParamsAAC;/* = {
    quantStepsSbr,
    quantStepsOrig,
    nrgBorders,
    4,
...
static __thread DETECTOR_PARAMETERS detectorParamsAACSpeech; /* = {
    quantStepsSbr,
    quantStepsOrig,
    nrgBorders,
    4,
  ...
  for(j = startIndex ; j < stopIndex ; j++){
    ...
    for(i = startChannel; i < stopChannel; i++) {
      ...
      if(indexVector[i] != -1)
      {
        ...
        quotaVecSbr[i] += (quotaMatrixOrig[j][(int)indexVector[i]]);
      }
    }
    INDIRECT(1); ADD(1); STORE(1);
